#Adds coordination number of each particle to fourth column of xyz file of minima selected. Also saves png of voronoi diagram
...
for minimum in range(inputminimum, inputminimarange+1):
	...
	for i in range(((numparts*minimum*2) - (2*numparts)), (numparts*minimum*2 - numparts)):
		wantedcoordslist3d.append([coordslist[i][0], coordslist[i][1], coordslist[i][2]])
...
for minimum in range(0, len(minimalist)):
	...
	#reglist = []
	coordinationlist = []
	for i in range(0, len(minimalist[minimum])):
		coordinationlist.append(0)
				...
				if (regionlengths != []) and (len(vordiag.regions[region]) == 6):
					...
					if notlargepolygon:
						if distortedpolygon:
							...
							coordinationlist[np.where(vordiag.point_region == region)[0][0]] = -1 
							if m<1:
							...
							polygon = [vordiag.vertices[i] for i in vordiag.regions[region]]
							coordinationlist[np.where(vordiag.point_region == region)[0][0]] = 6 
							if n<1:
	...
	#colouring polygons of 5 and 7 and other length sides
					...
					polygon = [vordiag.vertices[i] for i in vordiag.regions[region]]
					coordinationlist[np.where(vordiag.point_region == region)[0][0]] = 5 
					if k<1:
					...
					polygon = [vordiag.vertices[i] for i in vordiag.regions[region]]
					coordinationlist[np.where(vordiag.point_region == region)[0][0]] = 7 
					if q<1:
	...
	print(len(coordinationlist), coordinationlist)

	#--------
	#creating output xyz
	outputfile = str(numparts)+' \n'+'text \n'

	#Constructing output file

	for i in range(0, numparts):
		output = 'C ' + str(wantedcoordslist3d[i][0]) + ' ' + str(wantedcoordslist3d[i][1]) + ' ' + str(wantedcoordslist3d[i][2]) + ' ' + str(coordinationlist[i]) + '\n' 
		outputfile = outputfile + output

	print(filename+"withcoordination.xyz file produced")

	converted_file = open(filename+"_min"+str(minimum+1)+"_withcoordination.xyz", "w+")
