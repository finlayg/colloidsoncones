#script to quantify bond tension/compression from a given lowest input file and plot histogram of bond lengths
				...
				if (regionlengths != []) and (len(vordiag.regions[region]) == 6):
					...
					if notlargepolygon:
						if distortedpolygon:
							...
							if n<1:
								ax.fill(*zip(*polygon), "k", label="Regular hexagonal environment")
								n=1
								greenindexlist.append(region)
							else:
								ax.fill(*zip(*polygon), "k")
								greenindexlist.append(region)
...
#2 find each of its neighbours and calculate vector + distance to it
#3 store distances in a list
#4 plot lengths in a histogram
#5 plot lengths on diagram with colour gradient to show compression/tension

rmoutliers = input('Remove distance outliers? Y/N ')
if rmoutliers is 'Y':
	outlierthreshold = float(input('Remove outliers above: '))
...
#Doing bonds from particle to relevant neighbour

drawbonds = input('Add visualisation of bond lengths to structure diagram? Y/N ')
colourgradient = input('Colour bonds with colour gradient? (N gives discrete sections) Y/N ')

#normalising colour gradient
eqbmlength = 1.0
#norm = plt.Normalize(min(distancelist), max(distancelist))
#norm = plt.Normalize(eqbmlength-(max(distancelist)-eqbmlength), max(distancelist))
norm = plt.Normalize(eqbmlength-((max(distancelist)-eqbmlength)/2), ((max(distancelist)-eqbmlength)/2)+eqbmlength)
					...
					if (vertexindex in vordiag.regions[pg]) and (pg is not greenindexlist[gpg]):					#if it neghbours the first polygon
						...
						if (count is 2):											#and it isn't the first polygon
							...
							#HERE IS SETTING ARROW COLOURS
								...
								if colourgradient is 'Y':
									arrowcolour = plt.cm.get_cmap("coolwarm_r")(norm(distance))

								elif colourgradient is 'N':
							...
							#------------------------------------
								...
								plt.quiver(*origin, vector[0], vector[1], color = arrowcolour, zorder = 5, scale = 35, headwidth = 1)
...
if drawbonds is 'Y':
	fig.savefig('bondlengthhiststructbonds'+str(anglename)+'_min'+str(minimum+1)+'.png', dpi = 300)
	print('Minimum '+str(minimum+1)+' saved')
	fig.clf()
	plt.close()

elif drawbonds is 'N':
	fig.savefig('bondlengthhiststruct'+str(anglename)+'_min'+str(minimum+1)+'.png', dpi = 300)
	print('Minimum '+str(minimum+1)+' saved')
	fig.clf()
	plt.close()
...
#---------------------------------
#HERE IS PLOTTING HISTOGRAM + SETTING COLOURS

if colourgradient is 'Y':

#	centreofgradient = min(distancelist) + ((max(distancelist) - min(distancelist)) / 2)
#	left_data = [i for i in distancelist if i < centreofgradient]
#	right_data = [i for i in distancelist if i >= centreofgradient]
	
	fig, ax = plt.subplots()
	n, bins, patches = plt.hist(distancelist, bins=100)
#	nL, binsL, patchesL = plt.hist(left_data, bins=100)
#	nR, binsR, patchesR = plt.hist(right_data, bins=100)
...
#	cmL = plt.cm.get_cmap("YlOrRd_r")
#	cmR = plt.cm.get_cmap("YlGn")
	bin_centres = 0.5*(bins[:1]+bins[1:])
#	bin_centresL = 0.5*(binsL[:1]+binsL[1:])
#	bin_centresR = 0.5*(binsR[:1]+binsR[1:])
#	col = bin_centres - min(bin_centres)
#	col /= max(col)

#	norm = plt.Normalize(min(distancelist), max(distancelist))
#	print(min(distancelist), max(distancelist))
#	normL = plt.Normalize(min(left_data), max(left_data))
#	normR = plt.Normalize(min(right_data), max(right_data))
	for c, p in zip(bin_centres, patches):
		plt.setp(p, "facecolor", cm(norm(c)))
#	for c, p in zip(bin_centresL, patchesL):
#		plt.setp(p, "facecolor", cmL(normL(c)))
#	for c, p in zip(bin_centresR, patchesR):
#		plt.setp(p, "facecolor", cmR(normR(c)))

elif colourgradient is 'N':
	fig, ax = plt.subplots()
	n, bins, patches = plt.hist(distancelist, bins=200)
	ax.set_xlabel('Bond Length')
	ax.set_ylabel('Frequency')
...
savehist = input('Save histogram of bond lengths? Y/N ')
if savehist is 'Y':
	fig.savefig('bondlengthhist'+str(anglename)+'_min'+str(minimum+1)+'.png')
